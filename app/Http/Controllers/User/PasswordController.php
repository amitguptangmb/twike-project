...
/**
 * Port of user/changepassword.aspx(.cs). Note the original does NOT
...
class PasswordController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($changeFlag === 1) {
            return redirect()->route('user.login')->with('status', 'Password Changed Successfully');
        }
        if ($changeFlag === 2) {
            return redirect()->route('user.login')->with('error', 'Old Password not matched');
        }

        return redirect()->route('user.login')->with('error', 'Password not Changed Successfully! please try again');
    }
}
