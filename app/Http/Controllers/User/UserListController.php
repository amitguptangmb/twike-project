...
/**
 * Port of user/userlist.aspx(.cs) - the real ASP.NET landing page after
 ...
 *
 * Moved here from `DashboardController` (route name `user.dashboard`) -
 ...
 * panel" section). `user.dashboard` now points at that new KPI page;
 * this controller keeps the real, already-working "User List" page alive
 * at `user.users` so nothing that already depends on it breaks, even
...
class UserListController extends Controller
{
    public function index(Request $request)
    {
        $rows = StoredProcedure::call('Get_Agent_UserList', [
            '@AgentID' => (int) $request->session()->get('user.id'),
        ]);

        return view('user.users.index', ['rows' => $rows]);
    }
}
