...
/**
 * The partner-facing "Dashboard" shown in the User-panel screenshots (PAY
 ...
 * the full audit). `partner/userlist.aspx` - the page that WAS the real
 * landing page and IS in the source - is a plain user list, now moved to
 * `UserListController`/`partner.users`. This controller is a reconstruction
 ...
 *   its dropdown - passing this partner's own `session('partner.id')` is
 ...
 *   business logic) for the logged-in partner - no proc surfaces it and
...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        $holdingAmt = DB::table('UserMaster')->where('UserId', $partnerId)->value('Hold_Amt') ?? '0';
