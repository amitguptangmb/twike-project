...
/**
 * The partner-facing "Setting" page shown in the User-panel screenshots -
 ...
 * already-converted, already partner-scoped CRUD proc:
...
class SettingController extends Controller
{
    public function index(Request $request)
    {
        ...
        $business = DB::table('UserMaster')->where('UserId', $partnerId)->first();
        $tax = DB::table('User_Tax_Mst')->where('UserID', (string) $partnerId)->first();

        $whitelist = StoredProcedure::callWithOutputRows(
            'USP_CRUD_IP_WHITELIST',
            ['@flag' => 1, '@WhitelistIPId' => 0, '@UserID' => $partnerId, '@WhitelistIP' => ''],
            '@loginStatus'
        )['rows'];

        $webhook = StoredProcedure::callWithOutputRows(
            'USP_CRUD_WEB_HOOK',
            ['@flag' => 1, '@UserID' => $partnerId, '@Callback_URL' => '', '@Payout_Url' => ''],
            '@loginStatus'
        )['rows'][0] ?? null;
        ...
        $apiKey = StoredProcedure::callWithOutputRows(
            'USP_CRUD_API_KEY',
            [
                '@flag' => 1, '@UserID' => $partnerId, '@ClientID' => '', '@ClientSecret' => '',
        ...
        $revealedKeys = $request->session()->pull('partner.setting.revealed_keys');
    ...
    public function addIp(Request $request)
    {
        ...
        $result = StoredProcedure::callWithOutputRows(
            'USP_CRUD_IP_WHITELIST',
            ['@flag' => 2, '@WhitelistIPId' => 0, '@UserID' => $partnerId, '@WhitelistIP' => $ip],
            '@loginStatus'
        );
    ...
    public function deleteIp(Request $request, int $whitelistIpId)
    {
        ...
        $result = StoredProcedure::callWithOutputRows(
            'USP_CRUD_IP_WHITELIST',
            ['@flag' => 4, '@WhitelistIPId' => $whitelistIpId, '@UserID' => $partnerId, '@WhitelistIP' => ''],
            '@loginStatus'
        );
    ...
    public function regenerateApiKey(Request $request)
    {
        ...
        return redirect()->route('partner.setting')
            ->with('status', 'API key regenerated.')
            ->with('partner.setting.revealed_keys', $keys);
    }
    ...
    public function viewKeys(Request $request)
    {
        ...
        return redirect()->route('partner.setting')
            ->with('partner.setting.revealed_keys', (array) $row);
    }
}
