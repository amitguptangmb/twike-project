...
BEGIN
   ...
   select `ClientSecret`,`RequestHashKey`  FROM `TwikeDB`.`UserMaster`  where `ClientID` = v_ClientID;
END;
END
$$
...
BEGIN    
 ...
 from UserMaster as UserMasters    
   ...
   EmailId=v_UserId and    
         #  Password=cast(DecryptByPassPhrase( 'TWIKEv_234556LoginTest',v_Password) AS CHAR) and    
