...
@section('content')
                            ...
                            <tbody>
                                @forelse ($whitelist as $i => $ip)
                                    <tr>
                                        ...
                                        <td>{{ $ip->Created_On }}</td>
                                        <td>
                                            <form method="POST" action="{{ route('partner.setting.ip-whitelist.delete', $ip->WhitelistIPId) }}" class="d-inline">
                                                @csrf
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        <form method="POST" action="{{ route('partner.setting.api-key.regenerate') }}" onsubmit="return confirm('Regenerating will invalidate the current key immediately. Continue?');">
                                            @csrf
                                            <button type="submit" class="btn btn-primary">Regenerate Key</button>
                                        </form>
                                    </td>
                                    <td>
                                        <form method="POST" action="{{ route('partner.setting.api-key.view') }}">
                                            @csrf
    ...
    <div class="modal fade" id="addIpModal" tabindex="-1">
        <div class="modal-dialog">
            <form method="POST" action="{{ route('partner.setting.ip-whitelist.add') }}">
                @csrf
    ...
    <div class="modal fade" id="editWebhookModal" tabindex="-1">
        <div class="modal-dialog">
            <form method="POST" action="{{ route('partner.setting.webhook.update') }}">
                @csrf
                <div class="modal-content">
                    <div class="modal-header">
